    ...
    public function fields($fields = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($field->type == 'text') {
                ...
                $html .= form_input($field->unique_id, set_value($field->unique_id, ($field->value ?? false)), 'class="form-control tip" id="' . $field->unique_id . '" ' . ($field->required ? 'required="required" data-fv-notempty-message="' . $field->name . ' ' . lang('is_required') . '"' : ''));
                $html .= '</div>';
            } elseif ($field->type == 'url') {
                $html .= '<div class="form-group">';
                $html .= '<label for="' . $field->unique_id . '">' . $field->name . '</label>';
                $html .= form_input($field->unique_id, set_value($field->unique_id, ($field->value ?? false)), 'class="form-control tip" id="' . $field->unique_id . '" data-fv-uri="true" data-fv-uri-message="' . lang('input_invalid') . '" ' . ($field->required ? 'required="required" data-fv-notempty-message="' . $field->name . ' ' . lang('is_required') . '"' : ''));
                $html .= '</div>';
                ...
                $html .= form_dropdown($field->unique_id, $options, set_value($field->unique_id, ($field->value ?? false)), 'class="form-control tip select2" style="width:100%;" id="' . $field->unique_id . '" ' . ($field->required ? 'required="required" data-fv-notempty-message="' . $field->name . ' ' . lang('is_required') . '"' : ''));
                $html .= '</div>';
                ...
                $html .= '<label for="' . $field->unique_id . '">' . $field->name . '</label>';
                $cfoptions      = explode('|', $field->options);
                $selected_value = ($field->value ?? false);
                foreach ($cfoptions as $key => $value) {
