...
class Forums_model extends CI_Model
{
    ...
    public function get_posts($limit, $start, $topic_id, $skip_post_id)
    {
        ...
        $q = $this->db->get_where('posts', ['posts.topic_id' => $topic_id]);
        if ($q->num_rows() > 0) {
            foreach ($q->result() as $row) {
                $data[] = $row;
            }
    ...
    {
        $q = $this->db->get_where('categories', ['slug' => $slug]);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getChildrenCategories($parent_id)
    {
        ...
        $cat = $this->db->get_where('categories', ['parent_id' => $parent_id]);
        if ($cat->num_rows() > 0) {
    ...
    public function getFieldsData($topic_id)
    {
        ...
        $cat = $this->db->get_where('fields_data', ['fields_data.topic_id' => $topic_id]);
        if ($cat->num_rows() > 0) {
            foreach (($cat->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        $user_id = $this->session->userdata('user_id');
        $q       = $this->db->get_where('star_votes', ['topic_id' => $topic_id, 'user_id' => $user_id]);
        if ($q->num_rows() > 0) {
            $v = $q->row();
            return $v->stars;
        }
    ...
    {
        $user_id = $this->session->userdata('user_id');
        $q       = $this->db->get_where('votes', ['topic_id' => $topic_id, 'user_id' => $user_id]);
        if ($q->num_rows() > 0) {
            $v = $q->row();
            return $v->vote;
        }
    ...
    public function getTopicBySlug($slug)
    {
        ...
        $q = $this->db->get_where('topics', ['topics.slug' => $slug]);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
        if ($this->db->insert('complains', $data, ['id' => $data['topic_id']])) {
            if ($data['post_id']) {
                $this->db->update('posts', ['status' => 2], ['id' => $data['post_id']]);
            } else {
                $this->db->update('topics', ['status' => 2], ['id' => $data['topic_id']]);
            }
            return true;
        }
        return false;
    ...
    {
        if ($this->db->update('topics', ['views' => $views], ['id' => $topic_id])) {
            return true;
        }
        return false;
    }
