...
class Ajax_calls extends MY_Controller
{
    ...
    {
        $query   = trim(urldecode($this->input->get('query')));
        $threads = $this->db->select(['title', 'slug', 'category_slug'])->like('title', $query)->get('topics')->result();
        if (empty($threads)) {
            die(lang('no_match_found'));
        }
        $this->tec->send_json(['links' => $threads]);
    }
