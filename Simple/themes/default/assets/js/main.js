...
$(document).ready(function () {
  $("#forum-search").blur(function (e) {
    setTimeout(() => {
      $("#search-suggestions").hide();
    }, 100);
  });
  $("#forum-search").focus(function (e) {
  ...
  $("#forum-search").keyup(
    ...
    function (e) {
      $.ajax({
        url: site_url + "ajax_calls/suggestions",
        type: "GET",
        cache: "false",
        ...
        success: function (data) {
          ...
          if (data.links) {
            var ll = data.links.length;
            for (var i = 0; i < ll; i++) {
              $("#search-suggestions").append(
                "<a onclick='location.href='" +
                  data.links[i].category_slug +
                  "/" +
                  data.links[i].slug +
                  "' href='" +
                  site_url +
            ...
            $("#search-suggestions").append(
              "<a class='list-group-item list-group-item-action'>No match found.</a>"
            );
          }
        },
        error: function (data) {
          ...
          $("#search-suggestions").append(
            "<a class='list-group-item list-group-item-action'>" + data + "</a>"
          );
        },
      });
    }
  ...
  //   );
  $(".tip").tooltip();
  $("#ad_settings").submit(function (e) {
    e.preventDefault();
    let form = $(this).serializeArray();
    for (var i = 0; i < form.length; i++) {
      if (
        form[i]["name"] == "ad_thread_code" ||
        form[i]["name"] == "ad_sidebar_code" ||
        form[i]["name"] == "ad_sidebar2_code" ||
        form[i]["name"] == "ad_footer_code" ||
        form[i]["name"] == "footer_code"
      ) {
        form[i]["value"] = window.btoa(form[i]["value"]);
      }
    }
    $.ajax({
  ...
  $(".copy_api_key").click(function (e) {
    ...
    try {
      ...
      var status = successful
        ? "Key Copied"
        : "Please press ctrl+c or right click to copy the api key";
    } catch (err) {
      var status =
        "Oops, unable to copy, please press ctrl+c or right click to copy the api key";
    }
    $(this).parents(".list-box").find(".help_copy_api_key").text(status);
  });
  $(".msg_link td:not(:first-child, :last-child)")
    .css("cursor", "pointer")
    .click(function (e) {
      var page = $(this).parent("tr").attr("data-page");
      window.location.href =
        site_url +
        "messages/conversation/" +
        $(this).parent("tr").attr("data-id") +
        (page > 0 ? "?page=" + page : "");
    });
      ...
      success: function (data) {
        if (action == "up") {
          ...
          if (cast == 1) {
            parent
              .find(".total-down")
              .text(parseInt(parent.find(".total-down").text() - 1));
          }
          ...
          if (cast == 1) {
            parent
              .find(".total-up")
              .text(parseInt(parent.find(".total-up").text() - 1));
          }
  ...
  if (Settings.editor == "redactor") {
    $("#editor").trumbowyg({
      ...
      svgPath:
        base_url + "themes/default/assets/components/trumbowyg/ui/icons.svg",
      plugins: {
        upload: {
    ...
    $(document).on("click", ".quote-reply", function (e) {
      ...
      simplemde.value(
        "> __" + lang.quoting + " " + author + "__ " + quote_html.trim() + " "
      );
  ...
  $(document).on("change", ".parent_category", function (e) {
    if ($(this).val()) {
      var id = $(this).val();
      $.getJSON(site_url + "topics/get_child_categories/" + id).done(function (
        res
      ) {
        ...
        if (res.fields !== false) {
          $("#category_fields").empty().html(res.fields.html);
  ...
  $(document).on("change", "#child_category", function (e) {
    if ($(this).val() && $(this).val() != 0) {
      var id = $(this).val();
      $.getJSON(site_url + "topics/get_child_fields/" + id).done(function (
        res
      ) {
        if (res.fields !== false) {
          $("#child_category_fields").empty().html(res.fields.html);
          $(".select2").select2({ minimumResultsForSearch: 6 });
  ...
  $(document).on("click", ".user", function (e) {
    ...
    if (_this.next(".user-info").hasClass("load")) {
      if (uid) {
        $.get(site_url + "ajax_calls/get_user_details/" + uid).done(function (
          data
        ) {
          $(".user_" + uid)
            .next(".user-info")
...
$(window).load(function () {
  ...
  $.each(avatars, function () {
    var chr = $(this).data("name").toString().charAt(0).toUpperCase();
    $(this)
      .css({
...
$("#myModal").on("show.bs.modal", function (e) {
  ...
  $(
    "#editPost, #categoryForm, #pageForm, #badgeForm, #fieldForm"
  ).formValidation({
    framework: "bootstrap",
    excluded: ":disabled",
  });
...
function handleEmoji() {
  emojify.setConfig({
    img_dir:
      base_url +
      "themes/default/assets/components/trumbowyg/emojify/images/basic",
  });
  emojify.run();
}
