...
if (is_file($installFile)) {
    ...
    switch ($step) {
        ...
        if (!is_writeable($indexFile)) {
            $error = true;
            echo "<div class='alert alert-error'><i class='icon-remove'></i> Index File (index.php) is not writeable!</div>";
        }
        if (!is_writeable($configFolder)) {
            $error = true;
            echo "<div class='alert alert-error'><i class='icon-remove'></i> Config Folder (app/config/) is not writeable!</div>";
        }
        if (!is_writeable($configFile)) {
            $error = true;
            echo "<div class='alert alert-error'><i class='icon-remove'></i> Config File (app/config/config.php) is not writeable!</div>";
        }
        if (phpversion() < '7.0') {
            $error = true;
        ...
        if ($object->status == 'success') {
            if (isset($object->database)) {
                ...
                if ($database->update_db($dbdata, $_POST['username'], $_POST['code']) == false) {
                    ...
                    $finished = true;
                    if (!@unlink('../SF')) {
                        echo "<div class='alert alert-warning'><i class='icon-warning'></i> Please remove the SF file from the main folder in order to lock the update tool.</div>";
                    }
                }
            } else {
